...
def FA(x,y,cin):
    ...
    carry_out = XOR(XOR(gand(x,y), gand(y, cin)), gand(x, cin))
    sum = gor(gor3(x,y,cin), gand(x,gand(y,cin)))
...
def fourBitAdd(xxxx,yyyy):
    ...
    (f, zz) = FA(xxxx[0], yyyy[0], e)
    return (f, (zz, zy, zt, zo))
...
def test_fourBitAdd():
    ...
    assert (ww == (0, 1, 1, 0) and c == 0)
    c, ww = fourBitAdd(one, one)
    assert (ww == (1, 1, 0, 0) and c == 0)
    c, ww = fourBitAdd(three, three)
    assert (ww == (1, 1, 1, 0) and c == 1)
    print("test_twoBitAdd worked (but incomplete test)")
